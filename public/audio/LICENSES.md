# 행사 음원 출처와 이용조건

확인·다운로드: 2026-09-14. 외부 스트리밍 없이 아래 원본 파일을 함께 배포합니다.

## BGM — How are you 가야금

- 저작자/가야금 연주: 서예지. 원곡 HOW ARE YOU: 김재영.
- 제공: 한국저작권위원회 공유마당, 2023년 기증저작물.
- 작품: https://gongu.copyright.or.kr/gongu/wrt/wrt/view.do?menuNo=200020&wrtSn=13333652
- 이용조건: **기증저작물 자유이용** (라이선스 코드 98). CC0로 표기하지 않습니다.
- 공식 조건: 저작물명·저작자명·출처·이용조건을 표시하면 제한 없이 자유롭게 이용 가능. 따라서 출처를 유지하여 행사·상업적 이용 및 웹 배포에 사용합니다.
- 공식 안내: https://gongu.copyright.or.kr/gongu/main/contents.do?menuNo=200092
- 원본: `How are you 가야금ver.mp3`
- 프로젝트 파일: `public/audio/how-are-you-gayageum.mp3` (원본 그대로, 1,165,216 bytes)
- 표기문: **How are you 가야금 — 서예지, 공유마당, 기증저작물 자유이용(저작권 정보 표시)**
- 앱에서 낮은 음량으로 반복재생합니다. 음원 자체는 편집하지 않았습니다.

## 성공 효과음 — Correct Bell

- 저작자: Fupi
- 제공: OpenGameArt.org
- 작품: https://opengameart.org/content/correct-bell
- 이용조건: **CC0 1.0 Universal**. 행사·상업적 이용, 복제, 수정, 재배포 가능. 출처 표시는 의무가 아니지만 남깁니다.
- 라이선스: https://creativecommons.org/publicdomain/zero/1.0/
- 원본: `bell.wav`
- 프로젝트 파일: `public/audio/correct-bell.wav` (원본 그대로, 281,900 bytes)

공유마당 BGM의 출처 표기는 송출 화면 하단 및 이 파일에 포함되어 있습니다. 프로젝트를 다른 곳에 배포할 때도 해당 표기와 파일을 함께 유지하세요.


## 현재 BGM — 한가위 대항전 리듬 (144 BPM)
festival-music.cjs에서 직접 합성하는 프로젝트 오리지널 연주입니다. 기존 곡·녹음·외부 샘플을 사용하지 않습니다. 5음계 발현음과 북·장구를 연상시키는 합성 타악 리듬이며 실제 국악기 녹음은 아닙니다. 빌드 시 /audio/festival-game.wav 생성. 이전 가야금 파일은 예비 파일로 보존합니다.

## 박수 — Applause in a large hall or church
저작자 eXpl0it3r. CC0 1.0. 행사·상업적 이용·수정·재배포 가능.
https://opengameart.org/content/applause-in-a-large-hall-or-church
파일: public/audio/applause.wav

## 팡파르 — Win Fanfare
저작자 gchoc (Graciano Choc). CC0 1.0. 행사·상업적 이용·수정·재배포 가능.
https://opengameart.org/node/161644
파일: public/audio/fanfare.ogg
라이선스: https://creativecommons.org/publicdomain/zero/1.0/
확인일: 2026-09-15

박수 파일은 원본의 0.5초 지점부터 8초를 발췌하고 시작·끝에 페이드를 적용했습니다(CC0 허용).
