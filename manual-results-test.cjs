...
S=fresh();S.scores=[99,200,100];S.successes=[8,0,9];S.screen='finale';S.finaleIndex=4;let html=showHTML();assert(html.includes('최종 합산 점수'));assert(/mvp-winner[^]*?관장님팀[^]*?200/.test(html));assert(!html.includes('성공'));assert(!html.includes('미인정'));assert.equal((html.match(/🏆 MVP/g)||[]).length,1);
