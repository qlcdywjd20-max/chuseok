...
function advanceFlow(){const f=S.flow;if(!f)return;pushHistory();S.flow=null;
 ...
 else if(f.kind==='arrival'){const at=position(S.players[S.selected]);S.card=clone(S.bank.find(c=>c.id===at&&c.enabled)||null);S.chanceApplied=false;if(S.card){setScreen('intro');tone(['golden','bonus'].includes(S.card.type)?'golden':'intro');flowTo('content',1500);}else{S.notice='쉬어가는 칸입니다. 다음 팀도 함께 응원해요!';flowTo('finish',1600);}}
 else if(f.kind==='content')openContent();
 else if(f.kind==='chanceReveal'){S.chanceApplied=true;startTimer(S.card.duration||20);tone('special');}
 else if(f.kind==='answer'){setScreen('answer');tone('answer');}
 ...
 else if(f.kind==='finale'){S.finaleIndex++;if(S.finaleIndex<4)flowTo('finale',2600);}
 publish();
}
function beginFinale(){S.timer=null;S.busy=false;S.dice=null;S.finaleIndex=0;setScreen('finale');flowTo('finale',3500);tone('finish');}
...
function endTimer(){if(!S.timer||S.timer.ended)return;if(!isQuiz(S.card)){S.timer.running=false;S.timer.remaining=0;S.timer.ended=true;tone('end');publish();return;}S.timer.running=false;S.timer.remaining=0;S.timer.ended=true;tone('end');setScreen('camera');S.cameraUntil=gameNow()+3000;flowTo('answer',3000);}
...
function applyScore(success){if(S.scored||!S.card||!['answer','mission','chance'].includes(S.screen))return;S.timer&&(S.timer.running=false);pushHistory();S.scored=true;S.outcome=success?'success':'soft';if(success){changeScore(S.selected,pointsFor(S.card));S.successes[S.selected]++;S.communityAchieved=S.successes[S.selected]===8;tone('answer');}S.at=gameNow();flowTo('finish',S.communityAchieved?4000:2400);publish();}
...
function dispatch(a,v){if(!hasControl())return;
 ...
 case 'question':case 'nextQuestion':{const b=S.bank.filter(c=>c.enabled),c=a==='question'?b.find(c=>c.id===Number(v)):b[(b.findIndex(c=>c.id===S.card?.id)+1)%b.length];if(!c)return;pushHistory();S.card=clone(c);S.timer=null;S.scored=false;S.outcome=null;S.roundOpen=true;S.phase='next';S.flow=null;setScreen('intro');flowTo('content',1500);}break;
 ...
 case 'finale':if(!confirm('현재 점수로 최종 결과를 발표할까요?'))return;pushHistory();beginFinale();break;
...
function showHTML(){const c=S.card,cat=category(c),team=TEAMS[S.selected],quiz=isQuiz(c),shell=(body,extra='')=>`<section class="show ${extra}" style="--cat:${cat[1]}">${body}</section>`;
 ...
 if(S.screen==='finale'){const result=TEAMS.map((t,i)=>({i,earned:S.scores[i],success:S.successes[i],score:S.successes[i]>=8?S.scores[i]:0})).sort((a,b)=>b.score-a.score||TEAM_ORDER.indexOf(a.i)-TEAM_ORDER.indexOf(b.i));result.forEach(r=>r.rank=1+result.filter(x=>x.score>r.score).length);return shell('<div class="eyebrow">오늘의 공동체 골든벨 결과</div><h2>'+(S.finaleIndex===0?'우리 팀, 함께 이뤄낸 도전!':S.finaleIndex===4?'함께해서 더 큰 복(福)!':'최종 순위를 발표합니다!')+'</h2><div class="podium">'+result.map((r,n)=>'<div class="'+(S.finaleIndex>0&&S.finaleIndex<=n?'unrevealed':'')+'" style="--team:'+TEAMS[r.i].color+'"><span>'+(S.finaleIndex===0?'도전 결과':['🥇','🥈','🥉'][r.rank-1]+' '+r.rank+'위')+'</span><h3>'+TEAMS[r.i].name+'</h3><strong>'+(S.finaleIndex===0?r.earned:r.score)+'<small>점</small></strong><p>성공 '+r.success+' / 8</p><p class="qualification">'+(r.success>=8?'공동체 미션 달성<br>최종 점수 인정!':'공동체 미션 미달성<br>최종 점수 미인정')+'</p></div>').join('')+'</div><p class="closing">'+(S.finaleIndex>=4?'함께해서 더 큰 복(福)! 우리 모두가 한 팀입니다.':'8문제 이상 성공한 팀의 획득 점수만 인정합니다.')+'</p>'+confetti(),'finale');}
 if(!c)return '';
...
function tick(){updateClock();if(!hasControl()||S.paused)return;const now=Date.now();if(S.flow&&now>=S.flow.due){advanceFlow();return;}if(S.timer?.running){const left=seconds();if(left!==lastClock){lastClock=left;if(left<=10&&left>0)tone('count');}if(left===0){pushHistory();endTimer();publish();}}}
