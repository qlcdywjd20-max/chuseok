...
function roll(steps,correction=false){
 ...
 const multiplier=!correction&&S.dieDouble?2:1;
 S.move={left:Math.abs(steps)*multiplier,direction:Math.sign(steps),correction};
 if(!correction)S.dieDouble=false;
 if(correction){flowTo('move',0);}else{S.roundOpen=true;S.dice={value:steps,multiplier,steps:steps*multiplier,rollingUntil:Date.now()+1200,showUntil:Date.now()+2200};flowTo('move',2200);tone('dice');}publish();
}
...
function advanceFlow(){const f=S.flow;if(!f)return;pushHistory();S.flow=null;
 ...
 else if(f.kind==='nextTeam'){S.selected=TEAM_ORDER[(TEAM_ORDER.indexOf(S.selected)+1)%3];S.phase='ready';S.card=null;S.timer=null;S.dice=null;S.dieInput=null;S.dieDouble=false;S.notice='';S.turnAt=Date.now();setScreen('board');}
...
function dispatch(a,v){if(!hasControl())return;
 ...
 if(a==='dieDouble'){if(S.busy||S.paused||S.screen!=='board'||S.phase!=='ready')return;S.dieDouble=!S.dieDouble;publish();return;}
...
let controlKey='';
function renderControl(){if(mode==='display')return;for(const id of ['bankLiveStatus','bankDetailsStatus'])if($(id))$(id).textContent=bankMessage||'조작권을 이어받으면 구글시트를 자동으로 확인합니다.';const refreshButton=$('control').querySelector('[data-action=refresh]');if(refreshButton)refreshButton.disabled=fetching||!authority||S.busy;const key=JSON.stringify([S.screen,S.selected,S.busy,S.phase,S.card,S.scored,S.chanceApplied,S.scores,S.sound,S.bgmOn,S.bgmVolume,S.timer?.running,S.timer?.ended,history.length,authority,S.bankAt,apiMessage,S.paused,S.flow?.kind,S.dieInput,S.dieDouble,S.successes,S.completed]);if(key===controlKey&&$('adminActions'))return;controlKey=key;
