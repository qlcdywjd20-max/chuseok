...
function showHTML(){const c=S.card,cat=category(c),team=TEAMS[S.selected],quiz=isQuiz(c),shell=(body,extra='')=>`<section class="show ${extra}" style="--cat:${cat[1]}">${body}</section>`;
 ...
 if(S.screen==='finale'){
  ...
  return shell('<div class="eyebrow">한가위 골든벨 최종 결과</div><h2>'+(final?(mvp.length>1?'오늘의 공동 MVP!':'오늘의 MVP!'):'최종 합산 점수를 발표합니다!')+'</h2><div class="podium">'+result.map((r,n)=>'<div class="'+(S.finaleIndex>0&&S.finaleIndex<=n?'unrevealed ':'')+(final&&r.score===top?'mvp-winner':'')+'" style="--team:'+TEAMS[r.i].color+'"><span>'+(final&&r.score===top?'🏆 MVP':['🥇','🥈','🥉'][r.rank-1]+' '+r.rank+'위')+'</span><h3>'+TEAMS[r.i].name+'</h3><strong>'+r.score+'<small>점</small></strong></div>').join('')+'</div><p class="closing">'+(final?'함께해서 더 큰 복(福)! 우리 모두가 한 팀입니다.':'문제 점수와 보너스 점수를 모두 합산합니다.')+'</p>'+confetti(),'finale');
 }
 if(!c)return '';
