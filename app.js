...
function dispatch(a,v){if(!hasControl())return;
 if(a==='bgmToggle'){S.bgmOn=S.bgmOn===false;if(S.bgmOn)S.bgmStarted=true;publish();return;}if(a==='bgmVolume'){const n=Number($('bgmVolumeInput').value);if(Number.isFinite(n)){S.bgmVolume=Math.max(0,Math.min(100,n))/100;publish();}return;}if(a==='successTest'){tone('successTest');publish();return;}
