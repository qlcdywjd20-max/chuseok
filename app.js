...
function playTone(kind){if(!S.sound)return;try{ctx??=new (window.AudioContext||window.webkitAudioContext)();if(ctx.state==='suspended')ctx.resume();const notes={test:[523,659,784,1047],step:[660],dice:[330,440,550],intro:[330,440,660],count:[880],end:[440,330],answer:[523,659,784,1047],special:[392,523,659,784],golden:[523,659,784,1047,1319],finish:[523,659,784,1047,784,1047]}[kind]||[660];notes.forEach((hz,i)=>{const start=ctx.currentTime+i*.12,osc=ctx.createOscillator(),gain=ctx.createGain();osc.type=kind==='step'?'sine':'triangle';osc.frequency.setValueAtTime(hz,start);gain.gain.setValueAtTime(0,start);gain.gain.linearRampToValueAtTime(.09,start+.01);gain.gain.exponentialRampToValueAtTime(.001,start+.16);osc.connect(gain);gain.connect(ctx.destination);osc.start(start);osc.stop(start+.18);});}catch{}}
let audioReady=false,lastAudioId=null,audioSeen=false;
function tone(kind){if(!hasControl()||!S.sound)return;S.audioCue={id:crypto.randomUUID(),kind,at:Date.now()};}
function receiveSound(){if(mode!=='display')return;const cue=S.audioCue;if(!audioSeen){audioSeen=true;lastAudioId=cue?.id;return;}if(!cue||cue.id===lastAudioId)return;lastAudioId=cue.id;if(audioReady&&(!S.paused||cue.kind==='test')&&Date.now()-cue.at<4000)playTone(cue.kind);}
function installBroadcastAudio(){if(!['/','/game','/display'].includes(location.pathname.replace(/\/$/,'')||'/'))return;const button=document.createElement('button');button.id='enableBroadcastSound';button.textContent='🔊 송출 소리 켜기 · 시작 전 한 번 클릭';button.style.cssText='position:fixed;bottom:12px;right:16px;z-index:9999;padding:12px 22px;border-radius:24px;border:2px solid #cc972c;background:#fff3ce;color:#344840;font-size:20px;cursor:pointer';button.onclick=async()=>{try{ctx??=new (window.AudioContext||window.webkitAudioContext)();await ctx.resume();audioReady=ctx.state==='running';if(audioReady){playTone('answer');button.remove();}}catch{button.textContent='소리 활성화를 다시 눌러주세요';}};document.body.append(button);}
if(typeof window!=='undefined'&&!globalThis.__TEST__)window.addEventListener('load',installBroadcastAudio,{once:true});
...
function applyScore(success){if(S.scored||!S.card||!['answer','mission','chance'].includes(S.screen))return;S.timer&&(S.timer.running=false);pushHistory();S.scored=true;S.outcome=success?'success':'soft';if(success){changeScore(S.selected,pointsFor(S.card));S.successes[S.selected]++;S.communityAchieved=false;tone('answer');}S.at=gameNow();flowTo('finish',S.communityAchieved?4000:2400);publish();}
...
function dispatch(a,v){if(!hasControl())return;
 ...
 case 'finalReveal':if(S.screen!=='finalWait')return;pushHistory();S.flow=null;S.finaleIndex=4;setScreen('finale');tone('finish');break;
...
if(poll)setInterval(()=>{if(hasControl())void refreshBank({auto:true});},5000);
}
...
function showHTML(){const c=S.card,cat=category(c),team=TEAMS[S.selected],quiz=isQuiz(c),shell=(body,extra='')=>`<section class="show ${extra}" style="--cat:${cat[1]}">${body}</section>`;
 ...
 if(S.screen==='finale'){
  ...
  return shell('<div class="eyebrow">한가위 골든벨 최종 결과</div><h2>'+'최종 합산 점수'+'</h2><div class="podium">'+result.map((r,n)=>'<div class="'+(S.finaleIndex>0&&S.finaleIndex<=n?'unrevealed ':'')+(final&&r.score===top?'mvp-winner':'')+'" style="--team:'+TEAMS[r.i].color+'"><span>'+(final&&r.score===top?'🏆 MVP':['🥇','🥈','🥉'][r.rank-1]+' '+r.rank+'위')+'</span><h3>'+TEAMS[r.i].name+'</h3><strong>'+r.score+'<small>점</small></strong></div>').join('')+'</div><p class="closing">'+(final?'함께해서 더 큰 복(福)! 우리 모두가 한 팀입니다.':'문제 점수와 보너스 점수를 모두 합산합니다.')+'</p>'+confetti(),'finale');
 }
 if(!c)return '';
 const head=`<div class="quiz-head"><span class="challenger" style="background:${team.color}">${team.name} 도전!</span><span>${String(c.id).padStart(2,'0')}번 · ${cat[0]} · 성공 시 +${pointsFor(c)}점</span></div>`;
...
let controlKey='';
function renderControl(){if(mode==='display')return;for(const id of ['bankLiveStatus','bankDetailsStatus'])if($(id))$(id).textContent=bankMessage||'조작권을 이어받으면 구글시트를 자동으로 확인합니다.';const refreshButton=$('control').querySelector('[data-action=refresh]');if(refreshButton)refreshButton.disabled=fetching||!authority||S.busy;const key=JSON.stringify([S.screen,S.selected,S.busy,S.phase,S.card,S.scored,S.chanceApplied,S.scores,S.sound,S.timer?.running,S.timer?.ended,history.length,authority,S.bankAt,apiMessage,S.paused,S.flow?.kind,S.dieInput,S.successes,S.completed]);if(key===controlKey&&$('adminActions'))return;controlKey=key;
 ...
 <details id="bankDetails"><summary>Google Sheets 문제은행</summary><label>구글시트 또는 Apps Script 주소<input id="apiUrl" value="${esc(S.bankSource||safeRead(KEYS.api)||DEFAULT_BANK_URL||'')}" placeholder="https://script.google.com/macros/s/…/exec"></label>${b('문제 새로고침','refresh','',fetching)}<p id="bankDetailsStatus">${esc(bankMessage)}</p><p>이 창에서 조작권을 유지하면 5초마다 시트를 확인합니다. 문제·정답·유형·점수는 칸번호로 연결되며 송출 화면에도 반영됩니다.</p></details></div>`;
