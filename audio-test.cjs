...
(async()=>{
 ...
 let s={screen:'start',sound:true,bgmOn:true,bgmVolume:.12};audio.update(s);assert(bgm.paused);assert(bgm.loop);audio.unlock();await new Promise(r=>setImmediate(r));s={...s,screen:'board',bgmStarted:true};audio.update(s);await new Promise(r=>setImmediate(r));assert(!bgm.paused);assert.equal(bgm.volume,.12);
 ...
 audio.update({...s,audioCue:{id:'old',kind:'success',at:Date.now()-9000}});assert.equal(sfx.dataset.plays,'1');audio.update({...s,paused:true});assert(bgm.paused);
 ...
 console.log('PASS: local files, start gating, loop, 12% default, exactly-once success, duck 2.4%, restore, changed volume, stale cue, pause/mute/resume, missing-file fallback.');
