...
기존 **Google Sheets 문제 새로고침**을 유지했습니다. 담당자가 Apps Script API 주소를 입력하고 새로고침하면 불러온 문제은행이 Supabase에도 저장됩니다. API 주소는 담당자 브라우저마다 입력하지만 문제 데이터는 서버에 남습니다. Apps Script 배포·권한 설정은 별도로 완료해야 합니다. 연결 실패 시 마지막 저장 문제를 유지합니다.
